"""Error Catalog 對映（contracts/errors/error-catalog.yaml）。

所有 HTTP 錯誤回應必須符合 error-response.schema.json：
required = [error_code, message, correlation_id, retryable]；
禁止洩漏 stack trace、docker socket path、secret、host 絕對路徑。
"""
...
# RESOURCE_STATE delivery（保存於 AgentInstance.last_error_code，不走 HTTP 狀態碼）
RESOURCE_STATE_ERRORS = frozenset(
    {"RUNTIME_START_TIMEOUT", "RUNTIME_STOP_TIMEOUT"}
)
...
class ControllerError(Exception):
    ...
    def __init__(self, error_code: str, details: dict[str, str | int | bool | None] | None = None):
...
def error_response(error: ControllerError) -> JSONResponse:
